...
if skipGUI == True: # this condition means both are given in command line
    ...
    b1 = tkinter.Button(master, text="Run the Program", font="Helvetica 12 bold italic", command=setOwner, width=20).grid(row=3, column=0,sticky="ew", pady=4, padx=4) # add a single button to the popup
...
file = open(logPath.get() + 'ContactToTDSBLead-' + time + '.txt', 'w+')  # create and open the log file for this session
