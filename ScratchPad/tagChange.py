...
This module will change the users tags
...
logic will then be added to delete previous tags and add "M/H Phone" tag as needed
...
with open(csv_path, encoding="utf8", newline='', errors = 'ignore') as csvfile: # open files as UTF-8
    ...
    for row in reader:  # iterate through each row (record)
        ...
        #let's take a peak at the tags for debugging  
        APItags = response_json['data']['tags']
        
        #now lets clear the tags, for debugging (refactor our later)
        APItags = []
        response_json['data']['tags'] = APItags
        
        #get phone (mobile) and Home Phone
        if 'mobile' in response_json['data']:
            phone = response_json['data']['mobile']
        else:
            phone = None
        
        if 'Home Phone' in response_json['data']['custom_fields']:
            hPhone = response_json['data']['custom_fields']['Home Phone']
        else:
            hPhone = None
            
        #if phone and hPhone isn't null, add 'M/H Phone' tag
        if phone is not None or hPhone is not None:
            APItags.append('M/H Phone')
